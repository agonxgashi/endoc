...
router.get('/all/:userId', function (req, res) {
  ...
  Project.find({ $or: [{ 'CreatedBy': userId }, { 'Members': userId }] }, function (err, projects) {
    if (err) { res.send(err) }
  ...
  AppUser.findOne({ Username: memberId }, (err, user) => {
    if (err || !user) {
      ...
      Project.findByIdAndUpdate({ _id: projectId },
        ...
        .populate('Members', 'Name Surname Username')
        .exec((errE, p) => {
          if (errE) { res.send(new ReturnObj(false, 'ERR_MEMBER_NOT_REMOVED', 200, null)) } else { res.send(new ReturnObj(true, 'MSG_USER_ADDED_ON_BOARD', 200, p.Members)) }
        })
    }
  })
})

router.get('/remove-member/:projectId/:memberId', function (req, res) {
  const projectId = req.params.projectId
  ...
  Project.findByIdAndUpdate({ _id: projectId },
    ...
    .exec((errE, b) => {
      ...
      } else { res.send(new ReturnObj(true, 'MSG_USER_ADDED_ON_BOARD', 200, b.Members)) }
    })
})
